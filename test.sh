...
go run . -debug -seed 1234 -port 61960 &
sleep 2

for ((i=1;i<=$1-1;i++));
do
    if [ $# -eq 2 ] && [ $2 == "debug" ]; then
        go run . -debug -peer /ip4/127.0.0.1/tcp/61960/p2p/12D3KooWE3AwZFT9zEWDUxhya62hmvEbRxYBWaosn7Kiqw5wsu73 &
        sleep 1
    else 
        go run . -peer /ip4/127.0.0.1/tcp/61960/p2p/12D3KooWE3AwZFT9zEWDUxhya62hmvEbRxYBWaosn7Kiqw5wsu73 &
        sleep 1
    fi
done
